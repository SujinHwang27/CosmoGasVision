...
| **Stage 3** | Feedback classification on truth $\rho$ — **scope locked to option (c) probe-classifier-discriminability-lower-bound per [D-52] post-pre-review amendments 2026-05-13b; Δ̂(r) deferred to follow-on paper** | 🚀 **IN PROGRESS — [D-47] option-C′ (truth-only) per [D-52] dual-panel-overturn 2026-05-13a + post-pre-review amendments 2026-05-13b** — sprints 1–3 DONE ([D-48] cache, [D-49] split, [D-50] CIC refactor); sprint-4 ([D-51] truth-baseline 3D ResNet) implementation surface IN; **30-epoch full train CONDITIONALLY AUTHORIZED 2026-05-13b** post the [D-52] amendment package (Dispatch 5 LEDGER + Dispatch 6 design-doc sync + Dispatch 7 prior-work check) — gated on data-engineer dispatch to pull P2/P3/P4 IGM_gal from S3 sherwood-raw (PI ruled P1-only train NOT viable: AD-2/AD-3/AD-5 are 4-class-specific); **sprint-6 ([D-46] Tier-1 Juno) CANCELED 2026-05-12** after 50-step host smoke FAIL (3/7 gates, D4 signature; see [D-46] Addendum 2); 4/4 saturation-band-cascade closes within the [D-24] supervision regime (D1/D2/D3/D4 retired; supervision-target axis upstream-untested per [D-53]); **sprint-5 (reconstructed-baseline) reframed to option (c) per dual-panel overturn 2026-05-13a + amended 2026-05-13b**: ships Â_truth(r) as a **probe-classifier discriminability lower bound** (Alain & Bengio 2017; Theunissen 2003) on Sherwood physics-recipe signature at 3D ρ crop=32³ NeRF-substrate scale (novel; Bolton+2017 + Iršič+2017 cite-precedent at 1D flux-stat scale per amendment 12); Δ̂(r) demoted to follow-on (post-CVPR) campaign under option (b) with [D-53] supervision-target obligation. PI prior-session option-(a) recommendation SUPERSEDED 2026-05-13a; PI option-(c)-as-written sign-off SUPERSEDED 2026-05-13b by post-pre-review amendment package. | **Â_truth(r) at $r_{25}/r_{50}/r_{75}$** reported with **1k-bootstrap + block-bootstrap (block=64 vox) 95% CI side-by-side** (post-amendment 2; was "5-bootstrap" typo). FOUR pre-committed outcome branches per [D-52]: (i) CI lower bound at $r_{50} > 0.87$ above-bar; (ii) [0.85, 0.87] indistinguishable-from-bar per MDE 0.05 at 80% power; (iii) <0.85 with AD-5 PASS at ≥10pp → "below empirical reference" per [D-37]-ext R5; (iv) AD-1/gate-(c)/gate-1 FAIL or AD-5 FAIL → PROCESS-FAILURE / CEILING-DISQUALIFIED status, NO Â_truth publication (rule-7 protection per R14). 0.85 bar is project-internal per [D-36]; no external observational anchor. Δ̂(r) NOT a paper-claim quantity in this submission. | Sec 3 (Experiments) + Sec 4 (Next Steps acknowledgement) |
| **[D-43] Submission cut-sequence** | CVPR plan-of-record | ✅ **Step 1 (D-42 framing) DONE** · ✅ **Step 2 (multi-seed bootstrap = [D-44]) DONE host-side** · ✅ **Step 3 (A2 3D ξ) RESOLVED as retain-1D-proxy + §3 disclosure** · 🔄 **Step 4 (B-cycle) REOPENED 2026-05-13 with B7 §0 reorder + B8 §4.2 reframe per dual-panel-overturn** (see [D-43] Addendum 1) — B1–B5 base cycle remains DONE; **B8 ✅ DONE 2026-05-13a** (Dispatch 3 PI APPROVE; §4.2 reframe + 3 new cites landed); **Dispatch 4 dual-panel pre-review on [D-52] returned BLOCK 2026-05-13b** with 12-item amendment package; **Dispatches 5/6/7 LANDED 2026-05-13b** ([D-52] amendments + sprint-4 design-doc sync + Bolton+2017/Iršič+2017 cite-precedent); PI re-review APPROVE-WITH-CAVEATS post Stage-3-row touch-up; **sprint-4 30-epoch CONDITIONALLY AUTHORIZED** post data-engineer P2/P3/P4 IGM_gal pull; **B7 + B9 (latex-author paper propagation of amendments 3/10/11/12) DEFERRED** to next session post sprint-4 close · ✅ **C2 (Tier-4) RESOLVED as KEEP-DEFERRED with §4.2 invariance-strengthener** (§4.2 itself REWRITTEN via B8 — "invariance" verb retired per [D-37]-ext R9 invariance-verb-discipline; replaced with "underspecification of the [D-24] supervision regime") · ✅ **B3 DOI verification (9/9 new entries) DONE** · ⏳ **Step 5 (Submit) pending — 6 pre-existing bib TODOs + final PDF compile + page-count check + B7/B9 close + sprint-4 30-epoch close** | First draft of CVPR paper ready to submit | All sections (final pass) |
...
- **[D-37] Honest-Reporting Rule for Empirical Findings (2026-05-09, PI)** — Trigger: during the S5/S7 Cell B (no-cap) interim review at Step 13k/25k, Cell B trajectory was essentially identical to Cell A, and the finding was reported in two pre-packaged framings ("defense in depth" vs "methods novelty weakens") before the empirical observation itself was stated. This is spin-bias and is not allowed.
  ...
  **Extension 2 update (2026-05-13b, post-Dispatch-4 pre-review on [D-52]):** Two additional binding rules R13 + R14 surfaced by the convergent dual-panel BLOCK on the [D-52] option-(c) scope-lock. Banked here this session; rule-set-growth-rate discipline (per [D-45]) restrains banking to the minimum necessary set — three additional candidate rules ([D-54] power-calibration / [D-55] discipline-bar-change-as-amendment / [D-56] symmetric-disclosure framing test / [D-57] block-bootstrap pre-registration from the 3-examiner panel) are REJECTED-or-DEFERRED because R13 and R14 + the [D-52] post-pre-review amendment package cover the same surface with narrower, more operational scope. One additional candidate R15 (PI sign-off PROVISIONAL by default on stage-gate decisions) is DEFERRED pending operational test in next session.
  ...
  - **R15 candidate — PI sign-off PROVISIONAL by default on stage-gate decisions (DEFERRED, candidate rule)**: when PI sign-off touches deliverable-surface verbs or self-anchored bar promotions, the sign-off is **PROVISIONAL** by default; provisional status is lifted by (a) defense-panel pre-review APPROVE, or (b) explicit PI-only annotation with deferred-panel-review tracked in §7. Provisional status is binding on downstream dispatches: a downstream dispatch citing a PROVISIONAL sign-off as gate-prerequisite is itself dispatched provisionally. NOT banked this session per rule-set-growth-rate discipline ([D-45]); operational test = "did next session's PI sign-offs successfully use the PROVISIONAL annotation pattern without rotting?" If yes, bank R15 at session-after. Carry-forward in §7. Banking trigger: two consecutive convergent dual-panel overturns on PI-signed positions in the same session (2026-05-13a + 2026-05-13b) — signal not noise. Self-audit owed under [D-37] rule (c) honest-reporting.

  Propagated to `.claude/agents/project-architect.md` Extension 2 update block: R13 and R14 inscribed as binding; R15 inscribed as candidate-deferred-pending-operational-test (sibling to R12).
