...
| **Stage 3** | Feedback classification on reconstructed $\rho$ (gap measurement Δ̂(r)) | 🚀 **IN PROGRESS — [D-47] option-C sprint queue** — sprints 1–3 DONE ([D-48] cache, [D-49] split, [D-50] CIC refactor); sprint-4 ([D-51] truth-baseline 3D ResNet) dispatch-ready; sprint-6 ([D-46] Tier-1 Juno dispatch) unblocked post-quota-grant (2026-05-12), authorized for parallel dispatch with sprint-4; sprint-5 (reconstructed-baseline) downstream of both. **Reframed per [D-47]**: NOT gated on $P_F$-passing reconstruction; the headline scientific quantity is the gap Δ̂(r) = Â_truth(r) − Â_recon(r) on the 3D-structure-axis classifier surface. | Â_truth(r) anchors the [D-15] empirical 85% bar; Δ̂(r) at $r_{25}/r_{50}/r_{75}$ is the paper-claim quantity | Sec 4.3 (Next) — follow-up paper |
...
- **2026-05-12**: **Juno quota granted** (user confirmation). Unblocks [D-46] sprint-6 Tier-1 dispatch; sprint-4 + sprint-6 authorized for parallel execution (sprint-4 main-thread on local GPU; sprint-6 via `juno-hpc` skill on UTD HPC). T4 (792 GPU-hr) remains DEFERRED on scientific grounds (argument-from-invariance per paper §4.2), independent of the quota change.
...
### **Session Snapshot: 2026-05-12 (Stage 3 prep — sprints 1–3 close, paper fixes, sprint-4 next)**
...
- **Sprint-4 + sprint-6 PARALLEL DISPATCH** (authorized 2026-05-12 post-Juno-quota-grant):
  - **Sprint-4 [D-51] truth-baseline classifier** — main-thread on local GPU. PI design spec at `experiments/nerf/design/sprint4_truth_baseline.md` (§§1–12); [D-51] PENDING entry in §3. Implementation: `src/models/cnn3d.py` (NEW) + `src/analysis/conditional_accuracy.py` (NEW) + 4 anti-degeneracy test files. ~4–6 hr engineering + ~2.5 hr full train; gate eval ~5 min CPU.
  - **Sprint-6 [D-46] Tier-1 Juno dispatch** — `juno-hpc` skill. 4 physics × Tier-1 schedule per the [D-46] design spec (§3 [D-46] entry around line 927). MLflow round-trip to local tracker on completion. ETA: days range per [D-23] linear-VRAM × throughput projection.
- **Sprint-5 (reconstructed-baseline)** is downstream of BOTH sprint-4 and sprint-6 closing. Sprint-5 reuses sprint-4's `ResNet3D` architecture verbatim, with `crops` swapped from CIC-truth-ρ to NeRF-sampled-ρ produced by sprint-6's checkpoint. Gap Δ̂(r) = Â_truth(r) − Â_recon(r) is the post-sprint-5 paper-claim quantity.
...
- None. Sprint-4 + sprint-6 authorized for parallel dispatch.
