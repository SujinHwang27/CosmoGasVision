...
| **Stage 3** | Feedback classification on reconstructed $\rho$ (gap measurement Δ̂(r)) | 🚀 **IN PROGRESS — [D-47] option-C, partial cascade close** — sprints 1–3 DONE ([D-48] cache, [D-49] split, [D-50] CIC refactor); sprint-4 ([D-51] truth-baseline 3D ResNet) IN FLIGHT; **sprint-6 ([D-46] Tier-1 Juno) CANCELED 2026-05-12** after 50-step host smoke FAIL (3/7 gates, D4 signature; see [D-46] Addendum 2); 4/4 saturation-band counterfactual cascade closes (D1/D2/D3/D4 retired); **sprint-5 (reconstructed-baseline) BLOCKED on strategic call** about reconstruction source (pre-[D-46] PASS_T1_pub baselines vs new axis vs gap-unavailable). Reframed per [D-47]: NOT gated on $P_F$-passing reconstruction; headline scientific quantity is Δ̂(r) = Â_truth(r) − Â_recon(r). With [D-46] retired, Â_truth(r) alone is the [D-15] empirical ceiling deliverable from this sprint cycle; Δ̂(r) requires sprint-5 strategic resolution. | Â_truth(r) anchors the [D-15] empirical 85% bar; Δ̂(r) at $r_{25}/r_{50}/r_{75}$ is the paper-claim quantity (pending sprint-5 reconstruction-source decision) | Sec 4.3 (Next) — follow-up paper |
| **[D-43] Submission cut-sequence** | CVPR plan-of-record | ✅ **Step 1 (D-42 framing) DONE** · ✅ **Step 2 (multi-seed bootstrap = [D-44]) DONE host-side** · ✅ **Step 3 (A2 3D ξ) RESOLVED as retain-1D-proxy + §3 disclosure** · ✅ **Step 4 (B-cycle B1–B5) DONE** · ✅ **C2 (Tier-4) RESOLVED as KEEP-DEFERRED with §4.2 invariance-strengthener** · ✅ **B3 DOI verification (9/9 new entries) DONE** · ⏳ **Step 5 (Submit) pending — 6 pre-existing bib TODOs + final PDF compile + page-count check** | First draft of CVPR paper ready to submit | All sections (final pass) |
...
- **Sprint-4 [D-51] truth-baseline classifier** — IN FLIGHT, main-thread on local GPU. Implementation surface landed this session: `src/models/cnn3d.py` (NEW, ResNet-18 3D + 2 trivial baselines), `src/analysis/conditional_accuracy.py` (NEW, Â(r) + crop-unit bootstrap + audit-trail JSON), `src/data/augment3d.py` (NEW, per-sample-deterministic cube-isometry augmentation), 4 sprint-4 test files (3 PASS: cnn3d 10/10 + conditional_accuracy 15/15 + augmentation_label_independence 10/10 = 35/35; anti-leakage test pending run). PI design spec at `experiments/nerf/design/sprint4_truth_baseline.md` (§§1–12). [D-51] PENDING entry in §3. Remaining: pipeline driver + 5-step memory smoke + full ~2.5 hr train + 5-gate evaluation.
- **Sprint-6 [D-46] Tier-1 Juno CANCELED** per pre-committed stop after 50-step host smoke FAIL (see [D-46] Addendum 2 + §1 milestone). No Juno burn. 4/4 saturation-band counterfactual cascade closed (D1/D2/D3/D4 retired).
- **Sprint-5 (reconstructed-baseline) BLOCKED — strategic call needed.** With [D-46] retired, the originally-planned reconstruction source (sprint-6's [D-46] checkpoint) is unavailable. Three options surfaced for user/PI decision: (a) **pre-[D-46] PASS_T1_pub baselines** — use the 4 per-physics MLPs from the cost-survey (retired-with-P_F-failure but trained-and-available) as the reconstruction source. Most honest for the current paper since they're best-available trained reconstructions. (b) **propose a new axis intervention** beyond the 4 retired counterfactuals — would require new D-XX + design spec + smoke + panel review; not in this session's scope. (c) **accept gap measurement unavailable** — ship Â_truth(r) alone as the [D-15] empirical ceiling deliverable, defer Δ̂(r) to a follow-up campaign. Defense panel review must precede any sprint-5 dispatch on options (a) or (b). Recommend (a) for honest paper progress.
...
- **Sprint-5 (reconstructed-baseline) strategic call**: which reconstruction source to evaluate now that [D-46] is retired (options (a)/(b)/(c) above). PI sign-off 2026-05-12 recommends **option (a)** (pre-[D-46] PASS_T1_pub baselines) with mandatory hedging-language contract on any Δ̂(r) framing — gated on (i) sprint-4 numbers in hand, (ii) defense-panel post-mortem on the 4-cascade close, (iii) explicit sprint-5 D-XX. NOT this session.
- **Defense-panel post-mortem on the 4-cascade close + sprint-5 reconstruction-source decision: SCHEDULED for next session** per [D-37]-ext rule 6 (paper-claim threshold). Annotation **"PI-only, panel post-mortem scheduled"** applies to this session's [D-46] retirement record and the sprint-5 strategic deferral. Panel brief to dispatch: audit the D1/D2/D3/D4 axes for genuine orthogonality (is there a 5th — e.g., supervision-target axis on a saturation-aware τ transform — that we've missed?); confirm the §4.2 "argument-from-invariance" reads through with 4 axes retired; sign off on sprint-5 source (a)/(b)/(c).
- All other downstream sprints either complete (1–3), in-flight (4 — wiring-smoke this session, full-train next session), or canceled (6).
