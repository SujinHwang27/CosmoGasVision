...
- **2026-05-12**: **PI dispatch — sprint-3 [D-50] CIC chunked-bincount peak-memory refactor (in flight).** Promotes [D-48-followup] to formal D-XX after the sprint-2 regression run re-tripped the same OOM at $n_{\text{grid}}=64$ (157 MiB int64 cast on 20.6M particles). Owner: data-engineer. Gate: (a) slow $n_{\text{grid}}=768$ timing test PASSES within memory budget; (b) numerical equivalence `np.allclose(rho_new, rho_old, rtol=1e-5, atol=1e-7)`; (c) `test_rho_disk_cache.py` 5/5 PASS including the 3 currently-error tests; (d) sprint-1 + sprint-2 regression intact; (e) sprint-1 disk-cache manifest schema unchanged. Sprint-4 (3D ResNet truth-baseline, [D-47] option C step 1) **DEFERRED behind sprint-3** per PI dispatch ordering (sprint-4 + sprint-6 both hit the same OOM at production scale). Hedged framing per [D-37]-extension rule 2: chunked-bincount is the first-test of the fix, not a guaranteed resolution.
...
| **ρ-Field Disk Cache** | `Sherwood/.rho_field_cache/rho_field_p{id}_z{z:.3f}_n{n}.npy` + `.json` manifest | derived artifact, local-only (NOT git-tracked, NOT DVC-tracked); regenerable from snap_012 sim binaries via `SherwoodLoader.extract_rho_crops` cold path (~9 min/physics at $n_{\text{grid}}=768$); per [D-48] (2026-05-12). Manifest schema_version=1; validation includes snapshot mtime + sha256_first_1MB + mean $\in [0.95, 1.05]$ |
...
### **Session Snapshot: 2026-05-12 (Stage 3 prep — sprints 1–2 close, paper fixes, sprint-3 dispatched)**
...
- **Dispatch data-engineer for sprint-3 [D-50]** chunked-bincount CIC refactor. Gate per [D-50] entry (5 sub-gates including numerical equivalence).
- After [D-50] lands and PI signs off: invoke `ledger-update`; then dispatch sprint-4 (3D ResNet truth-baseline on truth ρ crops, [D-47] option-C step-1).
...
- Carry-forward (housekeeping): sweep two stale `*.tmp.npy` from `Sherwood/.rho_field_cache/`. Blocked on `Sherwood/**` deny rule for `Bash(rm:*)` and `Write/Edit`; defer to a permission grant or to the sprint-3 implementation pass.

### **Blockers**

- None for sprint-3 dispatch. Sprint-4 is blocked on sprint-3 close per PI ordering.
