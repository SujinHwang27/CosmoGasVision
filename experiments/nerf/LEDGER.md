...
- **Dispatch sprint-4 ([D-47] option-C step-1)**: 3D ResNet truth-baseline classifier on truth ρ crops with held-out region split. Now unblocked by [D-50] (CIC at $n_{\text{grid}}=768$ runs at 325 s / 4.49 GiB on a single physics) and [D-49] (anti-leakage split available). Sprint-5 ([D-46] physics-id-conditioned reconstruction MLP) and sprint-6 (Tier-1 Juno dispatch) follow.
