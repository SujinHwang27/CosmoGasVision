...
| **Stage 2b** | Full MLP Optimization | 🟧 **PASS_T1_pub: 2-of-3 [D-13] gates close at single-seed, P_F binding gate FAILS** — mean_flux 4/4 PASS at seed=42 single-seed @ $\langle F_{\text{pred}}\rangle \in [0.974, 0.984]$ (cross-physics spread 0.63%) but **2/4 at 5-seed sightline-level bootstrap** ($P_3$/$P_4$ marginal at q84; $P_1$/$P_2$ CIs entirely below the Kirkman anchor; bootstrap means lie 2–9$\sigma$ below seed=42 — high-$\langle F\rangle$-tail sampling, [D-44]); KS 3/4 PASS bootstrap-robustly ($P_2$ q84=0.0758, 1.5× over bar); $P_F$ 0/4 PASS ($\|\Delta P_F/P_F\|$ 36–42%, 3.6×–4.2× over 10% bar, 4/4 cells). [D-39] positively identifies saturation-regime under-fitting as residually-supported mechanism cross-physics ($R \in [18.87, 23.75]$, 4/4 cells $>12\times$ floor). Three counterfactual interventions tested under [D-37]-extension / [D-42-meta] anti-degeneracy discipline, **all retired**: **[D-40] sat-aware $P_F$ band loss FAIL** (constant-prediction collapse on integrated statistic, $+37.4\%$ worse); **[D-41] FGPA-tail per-pixel regularizer FAIL** at smoke + Tier-1 (constant-prediction collapse on self-consistent state); **[D-42] velocity-gradient conditioning FAIL at smoke** (gate 5 density-spread 0.0071, 200× below 1.45 floor; new degeneracy class = density-head collapse to ≈0 while X_HI head retains structure). **T4 (792 GPU-hr) DEFERRED on scientific grounds per [D-43] C2** — §4.2 argues the cross-physics-invariant saturation deficit is not addressable by sightline density; this is a scientific decision, **not compute-bound**. Juno quota granted 2026-05-12 (user confirmation): T4 is no longer compute-blocked but remains deferred on the argument-from-invariance until a different scientific case requires it. | $\|\Delta P_F/P_F\| < 10\%$ over $k_\parallel \in [10^{-2.5}, 10^{-1.5}]$ s/km AND $\xi_{\hat\rho,\rho}(r=2\,h^{-1}\,\text{Mpc}) > 0.6$ AND KS$(F\text{-PDF}) < 0.05$ at fiducial P1, $z=0.3$, $n_{\text{rays}}=1024$; degradation curve monotone over the $4 \times 4$ matrix. See [D-13], [D-39], [D-40], [D-41], [D-42], [D-43], [D-44]. | Sec 4.1 (Next) |
| **Stage 3** | Feedback classification on reconstructed $\rho$ — **scope locked to option (c) Â_truth(r) only per [D-52]; Δ̂(r) deferred to follow-on paper** | 🚀 **IN PROGRESS — [D-47] option-C′ (truth-only) per [D-52] dual-panel-overturn 2026-05-13** — sprints 1–3 DONE ([D-48] cache, [D-49] split, [D-50] CIC refactor); sprint-4 ([D-51] truth-baseline 3D ResNet) implementation surface IN; **30-epoch full train DEFERRED pending defense-panel pre-review on [D-52] scope-lock** (sprint-4 upgrade from instrument to paper-claim surface — see [D-52]); **sprint-6 ([D-46] Tier-1 Juno) CANCELED 2026-05-12** after 50-step host smoke FAIL (3/7 gates, D4 signature; see [D-46] Addendum 2); 4/4 saturation-band-cascade closes within the [D-24] supervision regime (D1/D2/D3/D4 retired; supervision-target axis upstream-untested per [D-53]); **sprint-5 (reconstructed-baseline) reframed to option (c) per dual-panel overturn 2026-05-13**: ships Â_truth(r) as the [D-15] empirical-ceiling deliverable, Δ̂(r) demoted to follow-on (post-CVPR) campaign under option (b) fresh-baseline reconstruction with [D-53] supervision-target obligation. PI prior-session option-(a) recommendation SUPERSEDED per panel verdict (STATS double-dipping + COSMO R-invariance confound). | **Â_truth(r) at $r_{25}/r_{50}/r_{75}$** anchors the [D-15] empirical 85% bar with crop-unit 1k-bootstrap 95% CI; pass = 5-bootstrap CI lower bound at $r_{50} > 0.85$ OR explicit "below-empirical-bar — establishes [D-15] floor at value X" reporting per [D-37]-ext rule 5. Δ̂(r) NOT a paper-claim quantity in this submission. | Sec 3 (Experiments) + Sec 4 (Next Steps acknowledgement) |
| **[D-43] Submission cut-sequence** | CVPR plan-of-record | ✅ **Step 1 (D-42 framing) DONE** · ✅ **Step 2 (multi-seed bootstrap = [D-44]) DONE host-side** · ✅ **Step 3 (A2 3D ξ) RESOLVED as retain-1D-proxy + §3 disclosure** · 🔄 **Step 4 (B-cycle) REOPENED 2026-05-13 with B7 §0 reorder + B8 §4.2 reframe per dual-panel-overturn** (see [D-43] Addendum 1) — B1–B5 base cycle remains DONE; **B8 ✅ DONE 2026-05-13** (Dispatch 3 PI APPROVE; §4.2 reframe + 3 new cites landed); **B7 IN FLIGHT** (gated on Dispatch 4 panel pre-review of [D-52], deferred to next session) · ✅ **C2 (Tier-4) RESOLVED as KEEP-DEFERRED with §4.2 invariance-strengthener** (§4.2 itself REWRITTEN via B8 — "invariance" verb retired per [D-37]-ext R9 invariance-verb-discipline; replaced with "underspecification of the [D-24] supervision regime") · ✅ **B3 DOI verification (9/9 new entries) DONE** · ⏳ **Step 5 (Submit) pending — 6 pre-existing bib TODOs + final PDF compile + page-count check + B7 close + [D-52] panel pre-review** | First draft of CVPR paper ready to submit | All sections (final pass) |
...
- **[D-51] Truth-baseline 3D ResNet classifier — sprint-4 ([D-47] option-C step-1) (2026-05-12, PI design spec + user sign-off)** — **PENDING**
...
- **[D-52] Sprint-5 reconstructed-baseline scope locked to option (c) truth-only — dual-panel-overturn of PI prior-session option-(a) recommendation (2026-05-13, PI + dual-panel)** — **DESIGN SPEC PENDING; SCOPE LOCKED**
  ...
  **Decision (scope-locked, not yet a full design spec)**: Sprint-5 ships **option (c) — Â_truth(r) alone** as the [D-15] empirical-ceiling deliverable. Δ̂(r) is DEMOTED to a future-work / sensitivity-subsection; not a headline paper-claim quantity in this submission cycle. Full design spec deferred to next session (gated on the defense-panel pre-review of this scope per Dispatch 4 below); this entry banks the scope-lock + the overturn evidence.
  ...
  **Sprint-5 success criterion (revised)**: Â_truth(r) at r_25 / r_50 / r_75 reports cross-physics 4-class held-out-region accuracy with crop-unit 1k-bootstrap 95% CI per [D-51] estimator. Strict-mode pass = single-number threshold per r-bin; the [D-15] 85% bar is *reported* but **not the gate** (panels confirmed [D-15] is project-side without external anchor — Â_truth(r) **is** the empirical anchor). Sprint-5 closes when:
  - 5-bootstrap CI lower bound at r_50 lands above 0.85 (over-empirical-bar reporting), OR
  - 5-bootstrap CI lower bound at r_50 lands below 0.85 with explicit "below empirical bar — establishes the [D-15] floor empirically at value X" reporting (per [D-37]-ext rule 5 symmetric-honesty).

  **Hedged framing (per [D-37]-ext rule 2, reinforced by new R10 venue-register discipline)**: under option (c), Â_truth(r) IS the headline-claim quantity (a change in discipline-bar from [D-51]'s "measurement instrument" framing). The 30-epoch sprint-4 dispatch is therefore upgraded to a paper-claim surface and must clear a defense-panel pre-review on the discipline-bar change before dispatch.

  **Δ̂(r) deferral**: Δ̂(r) is deferred to a post-CVPR follow-on campaign. The follow-on must clear option (b) (fresh-baseline reconstruction passing T1 evaluated on the [D-49] held-out region, with the gate-(a) closure protocol pre-registered BEFORE the held-out data is touched per [D-37]-ext rule 9 retired-model-reuse-contract discipline). No paper text in this CVPR submission claims a Δ̂(r) value.

  **Anti-degeneracy under option (c)**: AD-1–AD-4 from [D-51] all carry over. NEW AD-5 added: gate-(e) trivial-baseline backstop now serves a HEADLINE function (was only an instrument-discipline check under [D-51]); a 3D ResNet that fails to beat both trivial baselines by ≥5 pp is a falsified Â_truth(r) reading, not a soft-fail — escalates from instrument-disqualification to ceiling-disqualification.
  ...
  **References**: [D-15] (Stage 3 85% bar — empirical anchor under option (c)), [D-37]-ext (rules 2, 5, 6, 7; new rules R7–R10 banked this session per Extension 2 below), [D-39] (saturation-band positive ID — option (c) is the cleanest measurement of the ceiling against which the deficit is positively identified), [D-46] Addendum 2 (cascade close — the retirement context that overturns option (a) feasibility), [D-47] (option-C hybrid framing — Δ̂(r) demoted means option-C is now option-C′ "truth-only step-1, recon step-2 deferred"), [D-49] (held-out region — necessary for option (c), preserves anti-leakage), [D-51] (sprint-4 spec — supplies Â_truth(r) instrument under unchanged 5-gate), [D-53] (supervision-target upstream obligation — option (c) does not invoke a new intervention so does not trigger [D-53], but Δ̂(r) follow-on under option (b) WILL).

  **Review trail**: dual-panel verdict 2026-05-13 (3-examiner via dedicated agent + 4-persona via [D-42-meta] precedent agent) → PI re-orientation accepting overturn with one explicit pushback (4-persona's "pivot §4 to Stage 3 positive contribution" rejected: §4 stays "Next Steps" per CVPR architecture; sprint-5-Â_truth as positive contribution lives in §3 / §0 abstract, not in §4 next-steps; recorded as PI dissent for the record). Defense-panel pre-review on this scope-lock SCHEDULED as Dispatch 4 this session before sprint-4 30-epoch full-train authorization.
