...
- Ranking among (a)/(b)/(c) is PROVISIONAL pending panel; this doc presents all three for symmetric panel review, not a PI-pre-ranked recommendation.
...
## Candidate (a) — Iršič+2017-style flux-domain joint loss (PDF + P_F)

### 0. Literature audit + honest framing per [D-37] rule (a)

**Important caveat.** The reference "Iršič+ 2017 flux-domain loss" in the v5 stub points at Iršič et al. 2017 MNRAS 466:4332 (`arXiv:1702.01761`), which is the **XQ-100 P_F measurement paper** — it presents an observational power-spectrum measurement, not a loss-function formulation for neural-network supervision. The name "Iršič+ 2017 flux-domain loss" is a project-internal shorthand for "the joint flux-PDF + P_F supervision style that the Iršič+2017 / Boera+2019 measurement-paper tradition implies a downstream model should match." There is **no direct citation of a published flux-domain loss in that exact form for IGM-NeRF-class models** in the literature this researcher could locate. Closest precedents are summary-statistic-matching emulators (Cabayol-García et al. 2023 `arXiv:2305.19064`, Pedersen et al. emulator series) and field-level inference (Nayak/Maitra et al. 2023 `arXiv:2311.02167` "LyαNNA") — both **inference** machines targeting cosmological parameters from observed P_F + PDF, not **reconstruction** machines training a 3-D field against simulated PDF + P_F. Candidate (a) thus extrapolates from a measurement-paper convention into a loss; the panel should weight the absence of a direct precedent.

### 1. Mechanism prediction

**Proposed property of the target that produces non-degenerate gradients on the diffuse-bin majority:** the joint (flux PDF, P_F) target replaces a single τ-MSE-derived scalar field with two **distribution-level** summary statistics. The flux PDF p(F) over the unit interval is histogram-binned (typical 20-50 bins, e.g., Bolton+2008 convention); the gradient on each bin is non-degenerate so long as **any** voxel in the underlying sightline contributes to that flux bin under the differentiable Voigt path. The diffuse-bin majority — which carries τ ≪ 1, F ≈ 1 — concentrates near the PDF's F ≈ 1 bin, so a PDF-bin-MSE loss on that bin's height produces a coherent gradient signal across the diffuse-bin majority **rather than the per-voxel scalar imbalance that τ-MSE produces**.

**Why THIS target might rescue variance collapse where L1's τ-MSE-derived target did not (hedged):** the L1 sequence's failure mode was variance collapse of the predicted P_F band — the model converged on a near-constant flux that minimized τ-MSE locally but produced zero P_F variance. A PDF + P_F joint target **explicitly penalizes constant-flux solutions** (constant F has a delta-function PDF and a zero P_F at every k > 0), so the geometric structure of the loss surface around the variance-collapsed basin is presumably different. The literature does not specify this mechanism for neural reconstruction — it is the candidate's mechanism-prediction, not a citation.
...
**K2-equivalent for candidate (a):** a new `tests/test_torch_flux_pdf_estimator_equivalence.py` that asserts the **differentiable** `torch_flux_pdf(flux, n_bins)` produces histogram-bin heights matching a numpy reference `numpy_flux_pdf(flux, n_bins)` at `rtol=1e-4 / atol=1e-6` over 10 randomized batches (same seed-sweep as K2). Histogram differentiability requires a soft-binning convention (Gaussian or triangular kernel of width ~bin/2); the equivalence test certifies that the soft-binning approximation matches a hard-bin numpy reference within tolerance. **Pre-flight gate:** test must PASS before any Juno dispatch. Joint-loss test runs P_F estimator (K2 already certified) and PDF estimator independently — joint scalar is just the weighted sum, no separate equivalence needed.
...
- AVOIDED: "Iršič-style flux-domain losses are known to rescue this failure mode in IGM neural-net training" (would require literature citation that does not exist per §0).
...
- **Sbatch derived from** `scripts/submit_juno_sprint_L1_L2.sh` with two changes: (i) loss flag `--enable-l1-flux-domain` (new) replaces `--enable-l1-pf-loss`; (ii) wallclock relaxed from 24:00:00 to 04:00:00 since the smoke target is 200-step retire-or-pass, not 5k survival.
