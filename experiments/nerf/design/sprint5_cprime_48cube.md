...
**Status**: DRAFT v4 — PI-authored design doc; gate-2 RE-pre-review APPROVED-WITH-AMENDMENT at v3 absorbed 7 amendments inline; v4 absorbs 6 additional textual amendments (B1–B6) following the support-researcher pre-flight BLOCKER finding (sprint-4 `headline.json` lacks per-crop arrays needed for the v3 §4.3 S2 obligation); R23 BANKED as BINDING; sign-off NON-PROVISIONAL per [D-37]-Ext 2 R15 clause (b) carries over from v3 (v4 is textual absorption of a pre-flight finding, not a new attack surface — no third panel round).
...
## §3 R-rule audit per [D-37]-Extension 2 (R8–R23; R23 NEW at v4)
...
**R23 (NEW, BANK as BINDING; v4 amendment)**: **per-crop instrumentation requirement for AD-5-style pre-flights involving paired-classifier correlations.** Any AD-5-style pre-flight obligation that depends on paired-classifier correlations (ρ_emp, ρ_seed, or analogous joint-indicator correlations across architectures or seeds) must specify the per-crop instrumentation requirement at the spec-authoring time, NOT at the pre-flight time. Aggregate eval metrics (test accuracy, confusion matrix, bootstrap CIs over scalar accuracy) are insufficient to compute paired-classifier correctness correlations — those require per-crop correctness arrays (one indicator per (crop, classifier, seed) triple). The design doc spec must enumerate the per-crop instrumentation requirement before the predecessor run is dispatched; if the predecessor run did not bank per-crop arrays, the paired-correlation pre-flight cannot be discharged against that predecessor and must be re-routed to fire at the current-run's eval close (with its own per-crop instrumentation in place). Case-of-record: v3 §4.3 specced the S2 ρ_emp pre-flight against sprint-4 `headline.json` without verifying sprint-4 had banked per-crop correctness arrays — sprint-4 logged only aggregates. The 2026-05-15 support-researcher pre-flight returned BLOCKER. v4 §4.3 B1 amendment re-routes the obligation to (c′) eval close; v4 §5.1 B2 amendment adds the per-crop instrumentation requirement to the (c′) refactor. R23 binds future AD-5-style pre-flight specs to enumerate the per-crop instrumentation requirement at the spec-authoring layer, NOT at the pre-flight execution layer.
...
### §4.3 Empirical anchors for ρ + ρ_seed (S2 + A2 absorption; B1 v4 re-routing)

**ρ_emp obligation (S2; B1 v4 amendment — re-routed from pre-dispatch to (c′) eval close)**: v3 specced this obligation as a pre-dispatch compute from sprint-4's `headline.json`. The 2026-05-15 support-researcher pre-flight discharged that step and returned BLOCKER: `cloud_runs/Sprint4-30ep-a13dce8-20260514-110740-e72fca/eval/sprint4_1778774878_headline.json` contains aggregate accuracies + 4×4 confusion matrix + bootstrap CIs + gate verdicts, but no per-crop correctness arrays for either the ResNet or the `[mean, var]` baseline. The aggregate confusion captures `y_true × y_resnet_pred` but NOT the joint `1{ResNet correct} × 1{baseline correct}` needed for ρ_emp.

The B1 v4 re-routing: compute ρ_emp at **(c′) eval close** (post all 5 seeds), from the (c′) refactor's per-crop correctness logging (required for R23 BANK compliance — see §3 R23 below; instrumentation specified in §5.1 B2). Pair the per-crop correctness indicators between the (c′) 3D ResNet and the `[mean, var, skew, kurtosis]` 4-scalar baseline (the v3 AD-5 expansion from sprint-4's 2-scalar baseline). Compute over all 8000 test crops × 5 seeds = 40,000 paired observations OR seed-averaged at 8000 observations (both reported).

**Mid-run re-rule logic**: if ρ_emp ∉ [0.0, 0.5] AND the seed-averaged Â does not yet clear the AD-5 floor at the original MDE = 0.055, recompute MDE at the empirical ρ. If the recomputed MDE exceeds 0.10, the (c′) outcome is structurally undetermined — surface as **branch-(ii)-adjacent** (rerun with adjusted parameters), NOT branch-iv (genuine null). The S2 obligation does NOT block gate-4 dispatch under v4; it fires at gate-5 (outcome routing). v3's "if outside, recompute MDE at ρ_emp and re-rule" disposition is preserved with the discharge timing moved from gate-1 → gate-5.

**ρ_seed empirical recovery (A2 v3 addition)**: from first 2 (c′) seeds, computed as Corr(1{ResNet correct on crop i, seed s₁}, 1{ResNet correct on crop i, seed s₂}) over all 8000 test crops (paired across seeds, NOT across architectures). If ρ_seed_emp < 0.15, re-rule §4.2 at ρ_seed = 0; the MDE at ρ_seed = 0, k = 5 is 0.065 < 0.10, still applicable. Per R23 BANK, the per-seed per-crop correctness instrumentation enables this discharge.
...
| `experiments/nerf/pipeline.py` | Refactor to `run_sprint5_cprime_substrate_extension(crop_size=48, n_seeds=5)` **+ B2 v4: per-crop correctness instrumentation** | ~40 LOC delta + ~30 LOC B2. **B2 (R23-compliance)**: emit JSONL per seed at `cloud_runs/<run_tag>/eval/per_crop_seed_<N>.jsonl` with schema `{crop_idx: int, true_label: int, resnet_pred: int, mvsk_pred: int, m1_pred: int, resnet_correct: bool, mvsk_correct: bool, m1_correct: bool}`. Enables ρ_emp + ρ_seed discharge at (c′) eval close per §4.3 B1 re-routing. |
...
| `scripts/compute_rho_emp_sprint4.py` | **EXISTS (~120 LOC, BLOCKER disposition banked 2026-05-15)** — kept as evidence of the v3 S2 obligation BLOCKER finding; reads `headline.json`, surfaces "no per-crop arrays" BLOCKER, persists disposition to `experiments/nerf/artifacts/eval/sprint5_cprime/rho_emp_sprint4.json`. Per B1 v4 re-routing, the ρ_emp obligation now fires at (c′) eval close from the B2 per-crop JSONLs (NOT from sprint-4); the script remains as audit-trail evidence of the v3 spec-error. |
| `scripts/compute_sigma_seed_emp_cprime.py` | NEW — A1/A2 absorption; compute empirical σ_seed and ρ_seed from first 2 (c′) seeds' per-crop JSONLs (B2 v4 instrumentation); fires after seed 42 + 142 complete, before seeds 242/342/442 dispatch | ~50 LOC. |
| **B3 v4: MVSK-at-32³ instrumentation inside (c′) pipeline** (replaces external `scripts/compute_mvsk_baseline_sprint4.py`) | MVSK-baseline at sprint-4 32³ test set computed alongside MVSK-baseline at (c′) 48³ test set as part of the (c′) eval pipeline; sprint-4 32³ test crops re-extracted from Sherwood ρ at n_grid=768 on [D-49] split with seed=42 to match sprint-4's anchor; pipeline emits two MVSK numbers, A4 disclosure compares them per §6 gate-(e) footnote. | ~20 LOC inside `experiments/nerf/pipeline.py`. |
| `scripts/compute_mvsk_baseline_sprint4.py` | **EXISTS (~150 LOC, deferred-to-(c′) disposition banked 2026-05-15)** — kept as evidence of the v3 A4 pre-dispatch BLOCKER finding (no cached crop tensors at sprint-4); the (c′) refactor's B3 instrumentation supersedes it. Script remains as audit-trail evidence. |
...
| **(e)** | AD-5 trivial-baseline backstop (S3: 4-scalar; A4 v3 pre-flight on sprint-4 32³; B4 v4 amendment: A4 now in-(c′)-pipeline alongside MVSK-at-48³) | (e₁) `crop.mean()` 1-scalar → FC(4): Â ≤ Â(ResNet) − 0.10; (e₂) `[mean, var, skew, kurtosis]` 4-scalar → FC(4): Â ≤ Â(ResNet) − 0.10. Per-seed AND seed-averaged. **A4 INSTRUMENTATION (B4 v4 update)**: the (c′) eval pipeline computes MVSK-baseline at sprint-4 32³ test set alongside MVSK-baseline at (c′) 48³ test set in a single crop-extraction pass (sprint-4 32³ crops re-extracted from Sherwood ρ at n_grid=768 on [D-49] split with seed=42 to match sprint-4's anchor; (c′) 48³ crops extracted per the §4.1 spec). Pipeline emits both MVSK numbers. **If MVSK-at-32³ ≥ 0.42**, the 10 pp threshold has been silently tightened relative to sprint-4 [mean, var] = 0.368 baseline; disclose tightened bar in §6 footnote AND in any branch-iv interpretation. Tightening direction is interpretation-relevant: higher MVSK-at-32³ baseline makes the 10 pp gap to a 48³ ResNet *harder* to clear (more conservative for the design), not easier. v3's "pre-flight before (c′) dispatch" is retracted per the 2026-05-15 BLOCKER finding (no sprint-4 cached crops); B4 v4 unifies A4 + (c′) MVSK in one compute pass. | FAIL → branch-iv ceiling-disqualified. |
