...
| **Stage 2** | Baseline 3D U-Net training + corrected-metric eval | ✅ **G2 CLOSED — U-A CONFIRMED 2026-07-24 (S6 median-seed verdict).** Three-seed Juno Stage-2 run complete (42/142/242, a30); test r_s(σ=2,real,masked) = 0.9341/0.9402/0.9369 → **median seed 242 = 0.9369, beats grid bar 0.5758 by +0.361**; all five conditions fire for unet>grid (t=15.96), unet>wiener_L3 (t=17.50), unet>mlp (t=13.16); controls z1–z3 clean; nulls cleared; z4 = Δ_phys scoping column (pre-registered). U-A fires on the median AND unanimously on all three seeds. Verdict: `artifacts/stage2/s6_median_seed_verdict.json` | [U-06] §(d); G2 five-condition | **PAPER-RELICENSING TRIGGER under [D-75] §H — decision returns to the user.** PI disposition of the confirmed verdict owed |
...
- **2026-07-24**: Stage 0/1 ratified + built; Stage-2 3-seed Juno run (42/142/242) complete; **G2 U-A CONFIRMED on the median seed** (median r_s(σ=2)=0.9369 vs grid 0.5758). Paper-relicensing trigger fired; user decision owed.
...
- **[U-05] Benchmark-design survey banked — verdict: nothing to inherit; FORMALIZE OUR OWN as a candidate second paper product (2026-07-23; PROPOSAL status, user decision owed).**
  Full survey of record: `design/benchmark_survey_20260723.md` (support-researcher, WebSearch-verified citations). Load-bearing findings: (1) NO public (mock spectra, truth field, scoring code) benchmark exists anywhere in Lyα tomography — every paper self-scores on private mocks; DESI challenges are BAO/P1D-only; CLAMATO DR2 is truth-less real data; the Chaves-Montero 2026 review has no benchmark section (this ALSO discharges the [U-02]/[DP-02] review-scan obligation at survey scope — no neural-field/learned low-z tomography surfaced there). (2) DeepCHART's simulation suite CONFIRMED: own GADGET-3 runs with Sherwood-Relics physics, (40 h⁻¹Mpc)³ 2×512³, z=2.5 — "Sherwood-class" is the correct verb, "Bolton+2017 Sherwood" would be wrong ([U-02] citation-hygiene note satisfied). (3) The [D-75] suite already exceeds all prior practice on pre-registration, acceptance-tested estimators, and control fields; it lacks the community noise bundle, cosmic-web tier, multi-realization errors, a z≈2.5 bridge config, and public packaging. (4) Ranked top-5 build list in the design doc (noise/geometry package → in-house public baselines (dachshund, DeepCHART) → cosmic-web tier → public release engineering → cross-physics + z=2.5 axes). Status: PROPOSAL — becomes a work item only on user election; if elected, it is a benchmark-paper product ("first public benchmark for low-z / multi-physics Lyα tomography") orthogonal to the U-Net's win/lose, and the Stage-2 eval config should pre-adopt the inherited conventions (headline scales σ ∈ {1,2}; d⊥ ∈ {1.0, 2.4} reporting; noise ladder per the community bundle) so U-Net numbers are benchmark-grade from the first run.

- **[U-06] Stage-1 gate APPROVE + R9 absorption rulings + Stage-2 design spec (2026-07-23, PI; R15-PROVISIONAL, B1 panel lift owed pre-Juno).**
  Spec of record: `design/u06_stage2_spec.md` (committed verbatim; PI draft label "[U-05]" renumbered [U-06] at commit — [U-05] was taken by the benchmark survey). Load-bearing rulings: (1) MLP held-out verb of record = "statistically consistent with zero (0.073±0.094), spatially patchy" — NOT memorization (no train/test protocol existed for per-scene fits); only masked-vs-masked comparisons admissible henceforth; `unet>mlp` demoted to weak floor, interpretation weight on `unet>grid` + the U-B tie boundary. (2) Null = N=200 phase-rand distribution protocol (seeds default_rng([20260726,i])); "above null" = above 97.5th pct; the single-draw 0.1185 retired. (3) Operative G2 bars: grid 0.5758 (block SE 0.0443) / wiener_L3 0.0863 / mlp 0.0727 on the test mask; 0.5954 full-box = context anchor only; grid held-out ≈ full-box (|Δ|≤0.02 all σ) banked as slab-ceiling legitimation. (4) Architecture/loss/optimizer/smoke-gate numerics per spec §(a)-(d); GREEN at smoke = r_s ≥ 0.20 AND above null AND clean U-G controls — licenses Juno planning only, not a result claim. (5) G3 scope verb: "within-suite, shared-seed physics-response generalization" (byte-identical LOS geometry + shared ICs across variants). Dispatches on commit: core-implementer A1-A3+s2/s3; support-researcher A5 now + A4 after interfaces; infrastructure-manager A9; defense-panel B1.
