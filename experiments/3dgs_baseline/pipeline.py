...
if __name__ == "__main__":
    ...
    if use_mlflow:
        ...
        # Hierarchical experiment name per the mlflow-run skill
        mlflow.set_experiment("CosmoGasVision/3DGS")
        run_name = f"Stage1-Seeds-P{args.physics}-z{args.redshift:.3f}"
    ...
    with context_manager as run:
        if use_mlflow:
            # Mandatory tag set per the mlflow-run skill
            mlflow.set_tags({
                "model_type": "3dgs",
                "stage": "1",
                "physics_id": f"P{args.physics}",
                "redshift": f"{args.redshift}",
            })
            mlflow.log_params({
