...
cd ~/work
git clone <repo-url> CosmoGasVision
cd CosmoGasVision
pip install uv
...
WORK_DIR=~/scratch/stage2b/${RUN_TAG}
mkdir -p "${WORK_DIR}"
cd "${WORK_DIR}"

cp -r ~/work/CosmoGasVision/{src,experiments,scripts,pyproject.toml,uv.lock} .
ln -s ~/scratch/sherwood Sherwood        # sightlines + tauH1_*.dat (mirrored once, kept warm)
...
export MLFLOW_TRACKING_URI="file://${WORK_DIR}/mlflow"
export GIT_PYTHON_REFRESH=quiet           # silences GitPython deprecation in cloud logs
...
# --- 4. Copy out ---
DEST=~/work/stage2b_results/${RUN_TAG}
mkdir -p "${DEST}"
...
Submit per-cell with environment overrides:
...
1. **Pull from Juno to host**:

   ```bash
   rsync -avzP <NetID>@juno.utdallas.edu:~/work/stage2b_results/<RUN_TAG>/ \
       d:/Data/sujin/CosmoGasVision/cloud_runs/<RUN_TAG>/
   ```
