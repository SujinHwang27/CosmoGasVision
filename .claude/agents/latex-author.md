...
## User directive 2026-07-23 (binding): 6-page narrative + benefit gate

- **Length**: any CVPR-format cut is **strictly ≤ 6 pages** (user bar; tighter than the venue's 8-page hard limit — the user bar wins). Six pages carry a *narrative*, not an inventory: (1) which field and which problem this work addresses, (2) what is novel in our method for that field relative to prior work, (3) the quantifiable benefit the method brings. Any detail that does not serve this narrative moves to `sec_extended/`, it does not stay in the main paper.
- **Benefit gate**: paper iteration proceeds ONLY while the record contains at least one quantifiable benefit (speed, memory, a density-domain accuracy metric, or a community gate) over the paper baselines (TARDIS / Wiener) or a naive explicit baseline, with LEDGER provenance. If no such number exists, paper-fixing HALTS and the session escalates to the user with candidate metrics instead. Do not polish prose around a missing number.
- **Mental-model prose**: explanatory passages (intro, method rationale) are written as simple mental models — short declarative sentences a newcomer can follow; name the real technical term once, then explain it plainly; **strictly no analogies**. This extends the 2026-05-11 simple-prose directive; the "no CV/ML analogue commentary" anti-pattern below is a hard rule for all rendered prose.
- **Template of record**: `papers/cvpr2026/` uses the official `cvpr-org/author-kit` CVPR2026 release (`cvpr.sty` + `ieeenat_fullname.bst`), which is the latest published kit as of 2026-07 (no CVPR2027 kit released yet). Re-check https://github.com/cvpr-org/author-kit/releases at the next venue-manifest session before any real submission.
