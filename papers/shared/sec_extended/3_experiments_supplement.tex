...
The single-seed seed=42 anchor row backing the main bootstrap table (main Tab.~\ref{tab:headline-3gate}). Per the [D-44] symmetric-honesty disclosure, the 5-seed bootstrap mean for $\langle F_{\rm pred}\rangle$ lies systematically below this seed=42 anchor by $\meanFseedDeviationPone$ ($P_1$), $\meanFseedDeviationPtwo$ ($P_2$), $\meanFseedDeviationPthree$ ($P_3$), and $\meanFseedDeviationPfour$ ($P_4$): the single-seed reporting samples the high-$\langle F\rangle$ tail of the sightline distribution, while KS is not similarly biased.
...
\subsection{Substrate-discriminability probe at $32^3$: floor, CIs, per-quintile curve, confusion structure}
\label{sec:suppl:exp:substrate-probe-32cube}
Operational detail for the substrate-discriminability probe of \S\ref{sec:experiments:substrate-probe} (main paper), compressed there per the CVPR 8-pp cut and originating in [D-51] (sprint-4 truth-baseline 3D ResNet classifier, [D-47] option-C / [D-52] option (c)). The probe is a 3D ResNet-18 (halved channels, $\SubstrateProbeParams$ parameters) on $\SubstrateProbeCropSize^3$-voxel truth-$\rho/\bar\rho$ crops ($\SubstrateProbeVoxelKpc$ kpc/h voxels, a $\SubstrateProbeBoxMpc\,h^{-1}$ Mpc cube edge, matched to the $\xi_{\hat\rho,\rho}$ evaluation scale of \S\ref{sec:experiments:headline}), assigning each crop to one of the four Sherwood feedback variants $\{P_1, P_2, P_3, P_4\}$. The classifier accuracy is a lower bound on the cross-physics discriminability the substrate carries; the construction is read as a measurement of representational signal at this scale, not as a claim about Sherwood-recipe distinguishability per se.

\textbf{Anti-degeneracy floor.} Any signal the 3D ResNet exploits beyond the marginal one-point density distribution must clear a $\SubstrateProbeADfiveBarPP$ pp margin over the two-scalar $[\text{mean}, \text{variance}]$ MLP baseline (Geirhos et al.~\cite{geirhos2020shortcut} shortcut-learning audit). The $\SubstrateProbeADfiveBarPP$ pp margin is a project-internal disciplinary threshold with no external observational anchor; we adopt it by symmetry with the AdamW ablation precedent of Loshchilov \& Hutter~\cite{loshchilov2019adamw}, where sub-percentage-point accuracy differences are read as significant at benchmark-scale test sets ($n\!>\!50{,}000$), scaled to our $n\!\approx\!8{,}000$ Sherwood-crop test set.

\textbf{Result and confidence intervals.} The 3D ResNet reaches $\hat A_{\text{overall}} = \SubstrateProbeAcc$ on the 4-way task; the two-scalar baseline reaches $\SubstrateProbeMeanVarAcc$; chance is $\SubstrateProbeChance$. The margin is $\SubstrateProbeMarginPP$ pp, below the $\SubstrateProbeADfiveBarPP$ pp floor, so we decline to publish $\hat A_{\text{overall}}$ as a primary substrate-accuracy number. Evaluation is on $N_{\text{crops}} = \SubstrateProbeNcrops$ held-out crops from an axis-0 70/15/15 spatial split. Block-bootstrap and ordinary CIs on $\hat A_{\text{overall}}$ are $[\SubstrateProbeCIblockLo, \SubstrateProbeCIblockHi]$ (block size $\SubstrateProbeBlockVoxels$ voxels, $\sim\!2\times$ the gate scale~\cite{politis_romano1994}) and $[\SubstrateProbeCIordLo, \SubstrateProbeCIordHi]$ ($K{=}1{,}000$ resample) respectively; the maximum CI-lower-bound difference is $0.004$ ($\ll 0.02$), so the block correction does not change the conclusion. Per-quintile accuracies on the val-set-fixed equal-occupancy distance bins are $\hat A(r_{25}) = \SubstrateProbeAccQone$, $\hat A(r_{50}) = \SubstrateProbeAccQtwo$, $\hat A(r_{75}) = \SubstrateProbeAccQthree$ --- a flat curve, cross-quintile range $\sim\!\SubstrateProbeAccRangePP$ pp. The confusion matrix concentrates diagonal mass on the $P_1$ and $P_4$ rows, with $P_2$/$P_3$ near-chance off-diagonal spread: the extremes (no feedback vs.\ strong AGN) separate at this substrate while the middle (stellar wind vs.\ wind$+$AGN) does not.

\textbf{Prior-art context.} Cross-physics discriminability of the Sherwood $\{P_1$--$P_4\}$ variants is established at the 1D Ly$\alpha$-forest flux-statistic scale by the suite-overview comparisons in Bolton et al.~\cite{bolton2017sherwood} and by the cross-physics $P_F$ differentials in Iršič et al.~\cite{irsic2017xq100}. At the $\SubstrateProbeCropSize^3$ density-crop substrate matched to the gate scale and at the single resolution tested, the discriminative content the 3D ResNet recovers does not exceed what two summary moments capture by more than $\SubstrateProbeMarginPP$ pp; the probe at this substrate configuration does not surface higher-order signal beyond two summary moments.
...
\begin{table}[t]
    ...
    \hline
    mean  & $\SubstrateProbeCpResnetAcc$ & $\SubstrateProbeCpMvskAcc$ & $\SubstrateProbeCpMvskAtThirtyTwoCube$ & $\SubstrateProbeCpMarginPP$ \\
