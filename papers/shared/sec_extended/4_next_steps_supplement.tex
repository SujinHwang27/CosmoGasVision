...
\subsection{Substrate-discriminability successor-probe axes}
\label{sec:suppl:nextsteps:substrate-axes}
The substrate-discriminability probe (\S\ref{sec:experiments:substrate-probe}), which fell below its anti-degeneracy floor at the $\SubstrateProbeCropSize^3$ scale ([D-51]), makes three axes load-bearing for any successor probe; the main paper folds these into one sentence of \S\ref{sec:nextsteps:scope}. (a) \textbf{Substrate scale}: coarser crops or larger fields of view may recover the $P_2$/$P_3$ separation the present probe loses; finer features (temperature, $X_{HI}$, or peculiar-velocity gradients) may carry residual signal $\rho$ does not. (b) \textbf{Supervision-target alternatives}: a reconstructed-substrate probe would inherit any structural bias of the supervision regime, which the explicit-grid probe shows is under-constrained at $z = 0.3$~\cite{manheim_garrabrant2018}. (c) \textbf{Reconstructed-substrate evaluation}: the present probe was run on truth $\rho$; whether the $\sim\!1$ pp margin survives or widens on a reconstructed substrate is an open empirical question and the direct extension of this work toward a quantitative reconstruction-fidelity audit.
