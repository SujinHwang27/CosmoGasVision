...
\label{eq:data-loss}
\end{equation}
averaged over rays and unmasked velocity bins. The $\log(1+\tau)$ supervision space is defended on physical grounds: Lyman-alpha-forest opacity is approximately log-normally distributed in $\tau$ across the cool diffuse IGM~\cite{bi_davidsen1997, hui_gnedin1997}, a consequence of the Fluctuating Gunn-Peterson Approximation $\tau \propto \rho^{1.6} T^{-0.7}$ acting on a near-log-normal density PDF. The mask and cap are not part of the novelty claim; the supplementary records the Bolton--Haehnelt derivation of the $\tau_{\text{core}}=10^5$ threshold and a four-cell ablation showing they are not load-bearing for convergence on the tested configuration (retained as defense-in-depth against initialization-stress regimes; see [D-38]).
...
\end{equation}
at $\langle F\rangle_{\text{obs}}(z=0.3) = \AnchorMeanF \pm \AnchorMeanFsigma$ from Kirkman et al.~\cite{kirkman2007} ($D_A(z) = 0.016(1+z)^{1.01}$ evaluated at $z=0.3$), with $\lambda_F = 1.0$. The Sec.~\ref{sec:experiments} cost-survey runs were trained against an earlier broken anchor $0.877$ (a $z\!\sim\!2$ value mislabeled as $z=0.3$); the [D-34] retraction explains why those runs are kept (anchor-invariance of the [D-13] structure gates). The mean-flux constraint uses a two-pass surrogate: cycle-mean under \texttt{torch.no\_grad()}, then per-microbatch backward through a linearization $\mathcal{L}_{\text{mb}}=\mathcal{L}_{\text{data,mb}}+c\langle F\rangle_{\text{mb}}$ with $c=2\lambda_F(F_{\text{cycle}}-\langle F\rangle_{\text{obs}})$ held constant for the cycle. Derivation in supplementary.
